...
// setInterval(async () => {
    ...
    for (const cmd of commands) {
        ...
        if (status.code === 1) {
            const log = convert(`commit ${count} at ${getDate()}`) as Uint8Array;
            await Deno.writeFile('success.log', log)
        } else {
            const log = convert(`commit ${count} at ${getDate()}\n - ${error}`) as Uint8Array;
            await Deno.writeFile('error.log', log)
        }
...
function getDate() {
    ...
    return `${now.getHours()}:${now.getMinutes()} - ${now.getDate()}/${now.getMonth()}`;
}
